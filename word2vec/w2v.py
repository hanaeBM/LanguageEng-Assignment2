...
class Word2Vec(object):
    ...
    def get_context(self, i):
        ...
        context=[]
        for j in range(i-self.left_window_size,i+self.right_window_size+1):
            if j==i:
                continue
            if j<0 or j>=len(self.tokens):
                context.append(self.get_word_id(self.pad_word))
    ...
    def compute_sampling_distributions(self) :
        ...
        P_u, P_w = [], []
        self.freq[self.word2id[self.pad_word]]=1
        P=[[k,v] for k,v in self.freq.items()]
        P=sorted(P,key=lambda x:x[0],reverse=False)
        P=np.array([v for k,v in P])
        P_u=P/np.sum(P) 
        P_w=(P**0.75)/np.sum(P**0.75)
    ...
    def negative_sampling(self, number, focus, pos, distribution):
        ...
        """
        Taboo=[pos,focus]
        C=np.arange(len(self.id2word))
            
        idx=np.random.choice(C,p=distribution,size=number)

        for t in Taboo:
            while t in idx:
                I=np.where(idx==t)[0]
                idx[I]=np.random.choice(C,p=distribution,size=len(I))
    ...
    def train(self):
        ...
        while self.current_epoch < self.epochs:
            ...
            for i in tqdm(range(len(self.datapoints))):
                ...
                focus,C=self.datapoints[i]
                for pos in C:
                    grad_pos=self.w_vector[focus]*(self.sigmoid(self.w_tilde_vector[pos].T@self.w_vector[focus])-1)
                    grad_focus+=self.w_tilde_vector[pos]*(self.sigmoid(self.w_tilde_vector[pos].T@self.w_vector[focus])-1)
                    neg_samp=self.negative_sampling(self.no_of_neg_samples,focus,pos,distribution)
                    

                   
                    for n in neg_samp:
                        grad_neg=self.w_vector[focus]*(self.sigmoid(self.w_tilde_vector[n].T@self.w_vector[focus]))
                        grad_focus+=self.w_tilde_vector[n]*(self.sigmoid(self.w_tilde_vector[n].T@self.w_vector[focus]))
                        self.w_tilde_vector[n]=self.w_tilde_vector[n]-self.learning_rate*grad_neg

                    self.w_tilde_vector[pos]=self.w_tilde_vector[pos]-self.learning_rate*grad_pos

                self.w_vector[focus]=self.w_vector[focus]-self.learning_rate*grad_focus

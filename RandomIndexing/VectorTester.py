...
class VectorTester:
    ...
    def find_nearest(self, words, m='cosine'):
        ...
        nbrs = NearestNeighbors(n_neighbors=5, metric=m, algorithm='brute')
        
        
        nbrs.fit(self.vector)
        N=[]
        for w in words:
            w = w.lower().strip().strip(',')
            dist, idx = nbrs.kneighbors(self.vector[self.word2id[w]].reshape(1,-1))
            n = [(self.id2word[idx[0][i]], dist[0][i]) for i in range(5)]
            N.append(n)
...
def main() :
    ...
    parser.add_argument('--words', '-w', type=str, default=None)  

...
class RandomIndexing(object):
    ...
    def create_word_vectors(self):
        ...
        # random vectors
        for t in  range(len(self.id2word)):
            positions = np.random.choice(self.dimension, self.non_zero, replace=False)
            values = np.random.choice(self.non_zero_values, self.non_zero, replace=True)
            self.rv[t][positions] = values

        #context vector
        for id,context in self.datapoints:
            self.cv[id]+=np.sum(self.rv[context],axis=0)
